...
export default function ResultsPanel({ 
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const confidenceStr = typeof results.confidence === 'string' ? results.confidence : `${results.confidence || 0}%`;
      ...
      doc.setFontSize(9);
      doc.text(`REPORT ID: ${Date.now().toString()}`, 105, 40, { align: 'center' });
      ...
      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
      const notes = results.details?.notes || "This is an automated analysis based on pattern recognition algorithms.";
      ...
      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
      const recommendations = results.details?.recommendations || "Follow up with a healthcare professional for comprehensive evaluation.";
      // Split long text into multiple lines
      ...
      <div className="border-b border-gray-200 pb-5">
        <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
          ...
          <div>
            ...
            <dd className="mt-1 text-lg font-semibold">{results.confidence || '0%'}</dd>
          </div>
        </dl>
      </div>
      ...
      <div className="mt-6">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6">
          <div>
            ...
            <p className="mt-2 text-sm text-gray-500">{results.details?.notes || "This is an automated analysis based on pattern recognition algorithms."}</p>
          </div>
          <div>
            <h4 className="text-base font-medium text-gray-900">Recommendations</h4>
            <p className="mt-2 text-sm text-gray-500">{results.details?.recommendations || "Follow up with a healthcare professional for comprehensive evaluation."}</p>
          </div>
        </div>
        ...
        <div className="mt-6 flex flex-wrap gap-4">
          ...
          <Button onClick={handleDownloadPDF} disabled={isDownloading} variant="primary" className="bg-blue-100 text-blue-700 hover:bg-blue-200">
            {isDownloading ? 'Generating PDF...' : 'Download PDF Report'}
          </Button>
          <Button onClick={onPrintReport} variant="primary" className="bg-gray-100 text-gray-700 hover:bg-gray-200">
            Print Report
          </Button>
        </div>
      </div>
