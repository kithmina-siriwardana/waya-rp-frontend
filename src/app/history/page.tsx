...
export default function History() {
  ...
  const getPaginationInfo = () => {
    const startItem = filteredPredictions.length === 0 ? 0 : (currentPage - 1) * resultsPerPage + 1;
    const endItem = Math.min(currentPage * resultsPerPage, filteredPredictions.length);
  ...
  const handleDownload = async (prediction: Prediction) => {
    ...
    try {
      ...
      const lightGray = [189, 195, 199]; // Light gray for backgrounds
      ...
      doc.setFontSize(12);
      doc.text("BrainScan AI Medical Diagnostics", 105, 25, { align: 'center' });
        ...
        const tableData = prediction.details.regions.map(region => {
          ...
          return [
            ...
            { 
              ...
              styles: { 
                fontStyle: 'bold',
                textColor: region.score > 70 ? successColor : region.score > 50 ? [243, 156, 18] : warningColor
              } 
            }
          ];
        });
      ...
      doc.setFontSize(10);
      doc.setFont("helvetica", "normal");
      const recommendations = prediction.details?.recommendations || 'No recommendations provided.';
      // Split long text into multiple lines
      ...
      doc.text("This report is generated by an AI system and is for research purposes only.", 105, 280, { align: 'center' });
      doc.text("It is not a substitute for professional medical advice, diagnosis, or treatment.", 105, 285, { align: 'center' });
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-blue-500">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm text-gray-500 uppercase">Total Scans</p>
              ...
              <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-green-500">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm text-gray-500 uppercase">Positive Results</p>
                    <p className="text-xl font-bold">
                      {predictions.filter(p => p.prediction === "Non Demented").length}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-red-500">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm text-gray-500 uppercase">Negative Results</p>
                    <div>
                      <p className="text-xl font-bold">
                        {predictions.filter(p => 
                          p.prediction === "Moderate Dementia" || 
                          p.prediction === "Very Mild Dementia" || 
                          p.prediction === "Mild Dementia").length}
                      </p>
                      <p className="text-xs text-gray-500">
                        Moderate: {predictions.filter(p => p.prediction === "Moderate Dementia").length} | 
                        Mild: {predictions.filter(p => p.prediction === "Mild Dementia").length} | 
                        Very Mild: {predictions.filter(p => p.prediction === "Very Mild Dementia").length}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-4 border-l-4 border-purple-500">
                <div className="flex items-center">
                  ...
                  <div>
                    <p className="text-sm text-gray-500 uppercase">Avg. Confidence</p>
                    <p className="text-xl font-bold">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex items-center space-x-3">
                          ...
                          >
                            ...
                            {isDeleting === prediction._id ? 'Deleting...' : 'Delete'}
                          </button>
                        </div>
                      </td>
                    </tr>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                  <div>
                    <p className="text-sm text-gray-700">
                      {activeFilter !== "All" && (
                        <span className="mr-2">
                          <span className="font-semibold text-gray-900">{filteredPredictions.length}</span> {activeFilter} results found
                        </span>
                      )}
                      Showing <span className="font-semibold text-gray-900">{getPaginationInfo().startItem}</span> to{' '}
                      <span className="font-semibold text-gray-900">{getPaginationInfo().endItem}</span> of{' '}
                      <span className="font-semibold text-gray-900">{filteredPredictions.length}</span> results
                    </p>
                  </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <div className="flex items-center mb-4">
              ...
              <h3 className="text-lg font-medium text-gray-900">Delete Prediction</h3>
            </div>
          ...
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="mb-4">
                ...
                <p className="font-medium">{new Date(selectedPrediction.createdAt).toLocaleString()}</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedPrediction.confidence.toFixed(1)}%</p>
                </div>
              </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="border rounded-lg overflow-hidden">
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase">Region</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {selectedPrediction.details.regions.map((region, index) => (
                          <tr key={index}>
                            <td className="px-4 py-2 text-sm">{region.name}</td>
                            <td className="px-4 py-2 text-sm">{region.status}</td>
                            <td className="px-4 py-2 text-sm">{region.score.toFixed(1)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {selectedPrediction.details?.notes && (
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-700">{selectedPrediction.details.notes}</p>
                </div>
              )}
              ...
              {selectedPrediction.details?.recommendations && (
                <div className="mb-4">
                  ...
                  <p className="text-sm text-gray-700">{selectedPrediction.details.recommendations}</p>
                </div>
              )}
