...
export default function Dashboard() {
  ...
  const positiveResults = predictions.filter(p => p.prediction === "Non Demented").length;
  const negativeResults = predictions.filter(p => 
  ...
  // Stats data
  const stats = [
    { name: 'Total Scans', value: totalScans.toString(), icon: (
    ...
    { name: 'Positive Results', value: positiveResults.toString(), icon: (
    ...
    { name: 'Negative Results', value: negativeResults.toString(), icon: (
    ...
    { name: 'Avg. Confidence', value: avgConfidence, icon: (
            ...
            {stats.map((stat, index) => (
              <div key={index} className="bg-white rounded-lg shadow-md p-4 border-l-4 border-blue-500">
                <div className="flex items-center">
                  {stat.icon}
                  <div>
                    <p className="text-sm text-gray-500 uppercase">{stat.name}</p>
                    <p className="text-xl font-bold">{stat.value}</p>
                    {stat.name === 'Negative Results' && negativeResults > 0 && (
